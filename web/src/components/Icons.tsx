type IconsProps = {
  stroke?: string
  type: "settings" | "controller" | "play" | "pause" | "add" | "subtract"
}

function Icons(props: IconsProps) {
  const { type, stroke = "#9A9A9A" } = props

  return (
    <div>
      {type === "controller"
        ? controllerIcon(stroke)
        : type === "settings"
        ? settingsIcon(stroke)
        : type == "play"
        ? playIcon(stroke)
...
function addIcon(stroke: string) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="8" height="8" fill="none" viewBox="0 0 8 8">
      <path stroke={stroke} d="M0 3.944h8M3.944 8V0" />
    </svg>
  )
}

function subtractIcon(stroke: string) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="9" height="1" fill="none" viewBox="0 0 9 1">
      <path stroke={stroke} d="M0 .5h9" />
    </svg>
  )
}

function playIcon(stroke: string) {
  return (
...
function pauseIcon(stroke: string) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="none" viewBox="0 0 30 30">
      <rect width="12.273" height="30" fill={stroke} rx="3" />
      <rect width="12.273" height="30" x="17.727" fill={stroke} rx="3" />
    </svg>
  )
}

function settingsIcon(stroke: string) {
  return (
...
function controllerIcon(stroke: string) {
  return (
